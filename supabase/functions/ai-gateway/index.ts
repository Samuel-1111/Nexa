...
const TOOLS = [{ functionDeclarations: [
  { name: "create_task", description: "Create a task for the user", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, priority: { type: "STRING", enum: ["NONE", "LOW", "MEDIUM", "HIGH"] }, due_at: { type: "STRING", description: "ISO 8601 datetime, optional" } }, required: ["title"] } },
  { name: "create_reminder", description: "Create a reminder for the user", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, trigger_at: { type: "STRING", description: "ISO 8601 datetime, required, resolved from the user's timezone" }, timezone: { type: "STRING" } }, required: ["title", "trigger_at", "timezone"] } },
  { name: "create_note", description: "Save a note for the user", parameters: { type: "OBJECT", properties: { body: { type: "STRING" }, title: { type: "STRING" } }, required: ["body"] } },
  { name: "suggest_memory", description: "Suggest something worth remembering long-term. Never activates memory without user approval.", parameters: { type: "OBJECT", properties: { content: { type: "STRING" }, category: { type: "STRING", enum: ["PREFERENCE", "PERSON", "GOAL", "ROUTINE", "IMPORTANT_DATE", "WORK", "SCHOOL", "WRITING_STYLE", "OTHER"] } }, required: ["content", "category"] } },
  { name: "query_today", description: "Read the user's open tasks and upcoming reminders. Read-only.", parameters: { type: "OBJECT", properties: {} } },
] }];
...
Deno.serve(async (req: Request) => {
  ...
  // Voice transcription is an AI request and consumes one voice allowance.
  // Nothing is executed from the transcript until the Android client confirms it.
  if (body.audio_base64) {
    const usage = await consumeUsage(serviceClient, userId, true);
    ...
    const geminiResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${GEMINI_API_KEY}`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: transcriptionPrompt }, { inlineData: { mimeType, data: body.audio_base64 } }] }] }),
    });
  ...
  if (!body.message || typeof body.message !== "string" || body.message.length > 4000) return json({ error: "invalid_message" }, 400);

  // Text AI request. The quota is checked atomically in Postgres before Gemini is called.
  const usage = await consumeUsage(serviceClient, userId, false);
  if (!usage.allowed) return quotaResponse(usage);
  ...
  const geminiResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${GEMINI_API_KEY}`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ system_instruction: { parts: [{ text: systemInstruction }] }, contents: [{ role: "user", parts: [{ text: body.message }] }], tools: TOOLS }),
  });
  ...
  for (const part of parts) {
    ...
    if (part.functionCall) toolResults.push({ tool: part.functionCall.name, result: await executeTool(serviceClient, userId, part.functionCall.name, part.functionCall.args ?? {}) });
  }

  let audioBase64: string | undefined;
  let audioMimeType: string | undefined;
  if (body.speak && replyText.trim()) {
    const voiceUsage = await consumeUsage(serviceClient, userId, true, true);
    if (!voiceUsage.allowed) return json({ reply: replyText.trim(), tool_results: toolResults, voice_error: "voice_monthly_limit", usage });
    const audio = await synthesizeSpeech(replyText.trim());
    audioBase64 = audio?.data;
    audioMimeType = audio?.mimeType;
  }

  return json({ reply: replyText.trim(), tool_results: toolResults, audio_base64: audioBase64, audio_mime_type: audioMimeType, usage });
});

async function consumeUsage(client: any, userId: string, isVoice: boolean, voiceOnly = false) {
...
async function synthesizeSpeech(text: string): Promise<{ data: string; mimeType: string } | null> {
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_TTS_MODEL}:generateContent?key=${GEMINI_API_KEY}`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: "Kore" } } } } }),
  });
